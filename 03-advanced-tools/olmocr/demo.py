...
def pre_download_model():
    ...
    try:
        ...
        print(f"下載模型: {model_id} (FP8 量化版，記憶體需求較低)")
        cache_dir = snapshot_download(
            repo_id=model_id,
            local_files_only=False,  # 允許網絡下載
...
def start_sglang_server(model_path, port=30024):
    ...
    try:
        ...
        print(f"  🚀 啟動 SGLang server (記憶體優化配置)...")
        print(f"     模型: {model_path}")
        print(f"     記憶體設定: mem_fraction_static=0.2 (約 3.8GB KV cache)")
        ...
        # 啟動 SGLang server，使用最小化配置讓系統自動計算記憶體參數
        cmd = [
        ...
        print(f"  ⚙️  啟動命令: {' '.join(cmd[2:])}")  # 不顯示完整路徑
...
def convert_pdf(pdf_path):
    ...
    try:
        ...
        cmd = [
            ...
            "--model", model_path,  # 使用 Qwen2-VL 兼容版本
            "--model_max_context", "1024",  # 匹配 SGLang server 的 context 設定
        ]

        print(f"  執行命令: {' '.join(cmd[2:])}")  # 不顯示完整路徑
        print(f"  ⏱️  設定超時: 900秒")
        print(f"  📄 允許頁面錯誤率: 10%")
        ...
        try:
            ...
            if server_proc is not None:
                try:
                    ...
                    server_proc.terminate()
                    server_proc.wait(timeout=5)  # 等待5秒讓進程優雅退出
                except Exception as e:
                    print(f"  ⚠️  清理進程時出現問題: {e}")
                    try:
                        server_proc.kill()  # 強制終止
                    except:
                        pass

    except Exception as e:
        return {'success': False, 'error': f'整體流程失敗: {str(e)}'}
